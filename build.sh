...
${LIPO} -arch arm64 $lib_arm64 -arch x86_64 $lib_x86_64 -create -output $lib
done
}

mkdir -p $GLOBAL_OUTDIR/arm64_ios $GLOBAL_OUTDIR/arm64_sim $GLOBAL_OUTDIR/include/dcmtk
...
mv $IOS_OUTDIR/* $GLOBAL_OUTDIR/arm64_ios/

xcodebuild -configuration 'MinSizeRel' -sdk 'iphonesimulator' clean build ARCHS="arm64" ONLY_ACTIVE_ARCH=NO CONFIGURATION_BUILD_DIR=$SIM_OUTDIR
mv $SIM_OUTDIR/* $GLOBAL_OUTDIR/arm64_sim/

create_outdir_lipo

#rm -Rf $GLOBAL_OUTDIR/arm64 $GLOBAL_OUTDIR/x86_64
